...
// @usage - timeSince(new Date().setFullYear(2019))
 function timeSince(timeStamp) {
    ...
    var formatDate = function(date, format, utc) {
      ...
      var ddd = ["\x03", "Dom", "Lun", "Mar", "Mier", "Jue", "Vie", "Sab"];
    ...
    if(secondsPast < 60){ // Less than a minute
      return 'Hace '+ parseInt(secondsPast) + ' segs';
    }
    if(secondsPast < 3600){ // Less than an hour
      return 'Hace '+ parseInt(secondsPast/60) + ' mins';
    }
    if(secondsPast <= 86400){ // Less than a day
      return 'Hace '+parseInt(secondsPast/3600) + ' hrs';
    }
    if(secondsPast <= 172800){ // Less than 2 days
      return 'Ayer a las ' + formatDate(timeStamp, "h:mmtt");
    }
    if(secondsPast > 172800){ // After two days
      var timeString;

      if(secondsPast <= 604800)
        timeString = formatDate(timeStamp, "dddd") + " a las " + formatDate(timeStamp, "h:mmtt") // with in a week
        ...
        timeString = formatDate(timeStamp, "MMMM d") + " a las " + formatDate(timeStamp, "h:mmtt") // with in a month
